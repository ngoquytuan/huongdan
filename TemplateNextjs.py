...
'''
    ...
    def generate_template(self):
        ...
        print("\n" + "=" * 50)
        print("✅ HOÀN THÀNH TẠO TEMPLATE!")
        print("=" * 50)
        print(f"📍 Dự án được tạo tại: {self.base_path.absolute()}")
        print("\n🚀 Các bước tiếp theo:")
        print(f"1. cd {self.project_name}")
        print("2. npm install")
        print("3. cp .env.example .env.local")
        print("4. npm run dev")
        print("\n🐳 Hoặc chạy với Docker:")
        print("1. npm run docker:dev")
...
def main():
    ...
    print("🎯 Next.js Template Generator")
    print("Tạo cấu trúc dự án Next.js với TypeScript, Tailwind CSS và Docker")
    print("-" * 60)
    
    # Lấy tên dự án từ command line hoặc input
    if len(sys.argv) > 1:
        project_name = sys.argv[1]
    else:
        project_name = input("Nhập tên dự án (mặc định: my-nextjs-template): ").strip()
        if not project_name:
            project_name = "my-nextjs-template"
    
    # Validate tên dự án
    if not project_name.replace('-', '').replace('_', '').isalnum():
        print("❌ Tên dự án chỉ được chứa chữ cái, số, dấu gạch ngang (-) và gạch dưới (_)")
        sys.exit(1)
    ...
    if Path(project_name).exists():
        ...
        print(f"🗑️  Đã xóa thư mục cũ: {project_name}")
    ...
    try:
        ...
        print(f"❌ Lỗi khi tạo template: {str(e)}")
        sys.exit(1)
